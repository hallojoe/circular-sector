...
export function calculateArcPoints(center: IPoint, radius: number, startAngle: number, endAngle: number): [IPoint, IPoint, IPoint] {
  ...
  const startPoint = convertPolarToCartesian(center, radius, endAngle)
  const midPoint = convertPolarToCartesian(center, radius, startAngle + ((endAngle - startAngle) / 2))
  const endPoint = convertPolarToCartesian(center, radius, startAngle)
