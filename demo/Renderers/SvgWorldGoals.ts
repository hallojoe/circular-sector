...
export const svgWorldGoals = (): SVGSVGElement => {
  let last = 0
  const result = []
  const [distribution, distributionSum] = distribute([
  ...
  const svgPaths = svgPathCollection(result, 200, 5, 30, wordlGoalColors, 1, wordlGoalColors.map(x => x.replace(/1\)/, ".8)")), false)
